...
class ActionRequest {
            ...
            const streamPromise = new Promise((resolve, reject) => {
                if (url) {
                    ...
                    httpRequest
                        ...
                        .pipe(stream)
                        .on("error", (err) => {
                        winston.error(`[stream] PassThrough stream error`, Object.assign(Object.assign({}, this.logInfo), { error: err, stack: err.stack }));
                        reject(err);
                    })
                        .on("finish", () => {
    ...
    suggestedFilename() {
        if (this.attachment) {
            ...
            if (this.scheduledPlan && this.scheduledPlan.title) {
                return sanitizeFilename(`${this.scheduledPlan.title}.${utils_1.formatToFileExtension(this.formParams.format)}`);
            }
            else {
                return sanitizeFilename(`looker_file_${Date.now()}.${utils_1.formatToFileExtension(this.formParams.format)}`);
            }
        }
